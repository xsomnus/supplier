...
class ReadyCustomerOrderVC: BaseVC {
    ...
    func requestData() {
        ...
        if let cSupNo = userDefault.object(forKey: userDefaultUserIDKey) {
            provider.request(.URL_Select_Prepare_cSheetNo(cSupNo as! String, NSDate.getToday()!)) { (result) in
                switch result {
...
extension ReadyCustomerOrderVC:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //mark跳转到订单页面
        let VC = CustomerDidReadyDetailVC()
